...
router.post('/', async (req, res) => {
    try {
        ...
        const moto = await Motorcycle.create({ name, brand, cc, country, categoryId });
        res.status(201).json(moto);
    } catch (error) {
        res.status(500).json({ error: 'No s’ha pogut crear la moto' });
    }
});
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        if (!moto) return res.status(404).json({ error: 'Moto no trobada' });
        await moto.destroy();
        res.json({ message: 'Moto eliminada correctament' });
    } catch (error) {
